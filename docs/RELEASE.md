...
5. As the repository owner, run the `GPU release gate` and `Signed Windows
   package` workflows from `main` or the version tag. Both use the frozen Python
   lock and must succeed; unsigned output is not a release asset.
6. Trigger `Release artifacts` with the tag and the successful GPU and Windows
   run IDs. The workflow verifies the SHA, successful conclusion, dispatch event,
   and expected workflow path for both supplied runs.
