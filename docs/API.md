...
Source and output must resolve to different directories. Output may be a child of
source; the scanner excludes that resolved output subtree.
...
Browser WebSockets send the session token through a subprotocol and keep it out of
the URL. The protocol value is `vocalsieve.token.` followed by unpadded base64url
encoding of the UTF-8 token. The earlier query-token form remains a compatibility
path for non-browser clients:
...
its runtime connection form, keeps it only in memory, consumes only generated
OpenAPI types, and supports job creation, cancel/resume, results, reporting,
review, and re-export. It remains a development preview rather than a Windows
portable guarantee.
