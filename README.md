...
- Source and output paths must be different; output may be inside the source directory,
  and scanning excludes its subtree. Exports never modify the source corpus.
...
The source tree is preparing `0.9.0-rc.2`. No GitHub Release is currently
published, so there is no official download yet. CI artifacts and locally built
archives are not releases.
...
row-oriented CSV report, JSON report, and schema v2 aggregate summary. CSV text
cells that begin with spreadsheet formula markers are prefixed with an apostrophe;
JSON retains the original values.
...
Open `http://127.0.0.1:5173`, paste the token printed by `vocalsieve serve`, and
connect. The token stays in browser memory rather than the Vite bundle. The browser
is allowed only from the two documented localhost origins and does not receive
...
Project direction, decisions, review, releases, and repository history are maintained
solely by [@cheese-sansan](https://github.com/cheese-sansan).

Dependency updates are implemented on maintainer-controlled branches after the
weekly security workflow, a vulnerability alert, or release preparation identifies
a need. Automated dependency update pull requests are disabled.
