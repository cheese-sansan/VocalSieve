...
function App() {
  ...
  const exportSelectedJob = async () => {
    if (!selectedJob) return;
    try {
      const exported = await api.exportJob(selectedJob);
      setExportResult(exported);
      setActionMessage(`Exported ${exported.count} selected file(s).`);
      await loadJobDetails(selectedJob);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Could not export job");
    }
  };
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        <section>
          <div className="section-heading"><h2>Create job</h2><span>Local paths stay on this machine</span></div>
          <form className="job-form" onSubmit={createJob}>
            ...
            <button className="primary wide" disabled={isCreating} type="submit"><Play size={16} /> {isCreating ? "Creating..." : "Create and run job"}</button>
          </form>
        </section>
        ...
        <section>
          <div className="section-heading"><h2>Recent jobs</h2><span>Select a row to inspect events, results, and report</span></div>
        ...
        <section>
          <div className="section-heading">
            <h2>Job report</h2>
            <span>{selected ? selected.id.slice(0, 10) : "No job selected"}</span>
          </div>
          {report ? (
            <>
              ...
              <button className="primary" disabled={!selected} onClick={exportSelectedJob} type="button"><Download size={16} /> Export selected files</button>
              {exportResult && <p className="muted">Last export wrote {exportResult.count} file(s).</p>}
            </>
          ) : <p className="empty-state">Select a job to load its aggregate report.</p>}
        </section>

        <section>
          <div className="section-heading"><h2>Results</h2><span>{results.length} rows</span></div>
          <div className="table-wrap results-table">
            <table>
              <thead><tr><th>Path</th><th>Status</th><th>Reject</th><th>Duration</th><th>Score</th><th>Transcript</th></tr></thead>
              <tbody>
                {results.map((result) => (
                  <tr key={result.relative_path}>
                    <td>{result.relative_path}</td><td>{result.status}</td><td>{result.reject_code ?? "-"}</td>
                    <td>{formatNumber(result.duration)}</td><td>{formatNumber(result.score, 3)}</td>
