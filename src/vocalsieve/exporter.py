...
def _reject_linked_output_path(root: Path, relative: PurePosixPath | None = None) -> None:
    current = root
    candidates = [current]
    ...
    for candidate in candidates:
        if _is_linked_path(candidate):
            raise ValueError(f"Unsafe linked output path: {candidate}")
...
def safe_destination(root: Path, relative_path: str, *, reject_links: bool = False) -> Path:
    relative = PurePosixPath(relative_path)
    if relative.is_absolute() or ".." in relative.parts:
        raise ValueError(f"Unsafe relative path: {relative_path}")
    if reject_links:
        _reject_linked_output_path(root, relative)
...
def export_selected(
    ...
    *,
    ...
    _reject_linked_output_path(final_root)
    ...
    for row in rows:
        ...
        if not previous or row["relative_path"] in selected_paths:
            continue
        destination = safe_destination(final_root, row["relative_path"], reject_links=True)
    ...
    with csv_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writeheader()
        writer.writerows(
            {key: _csv_safe_value(row.get(key)) for key in REPORT_FIELDS} for row in rows
        )
