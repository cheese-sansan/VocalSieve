...
- 源目录与输出目录不可相同；输出目录可以位于源目录内，扫描时会排除该输出子树；
  导出过程不会修改源语料。
...
源码正在准备 `0.9.0-rc.2`。当前没有已发布的 GitHub Release，因此也没有官方可下载文件。
CI 产物和本地构建的压缩包都不属于 Release。
...
以电子表格公式标记开头的 CSV 文本单元格会添加单引号前缀；JSON 保留原始值。
...
打开 `http://127.0.0.1:5173`，粘贴 `vocalsieve serve` 打印的令牌并连接。令牌只保留
在浏览器内存中，不会写入 Vite 构建产物。浏览器仅允许从文档列出的两个 localhost
来源访问，也不会获得直接文件系统权限。
...
项目方向、决策、审查、发布与仓库历史仅由
[@cheese-sansan](https://github.com/cheese-sansan) 维护。

依赖更新由每周安全工作流、GitHub 漏洞告警或发布准备触发，并通过维护者控制的分支实施。
项目已关闭自动依赖更新 PR。
